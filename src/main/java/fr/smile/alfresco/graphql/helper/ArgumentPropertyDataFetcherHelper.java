 ...
 */
@Internal
public class ArgumentPropertyDataFetcherHelper {
    ...
    private static Map<Class<?>, Function<Object, Object>> argumentTransformers = new HashMap<>();
    static {
    	argumentTransformers.put(QName.class, o -> GraphQlConfiguration.getQName((String) o));
    }
        ...
        try {
            ...
            if (parameters.length == 0) {
				...
				for (Parameter parameter : parameters) {
					...
					if (type == DataFetchingEnvironment.class) {
	            		args.add(environment);
	            	} else {
	            		Object value = environment.getArgument(parameter.getName());
	            		if (value != null) {
		            		Function<Object, Object> transformer = argumentTransformers.get(type);
		            		if (transformer != null) {
		            			value = transformer.apply(value);
		            		} else if (type.isEnum()) {
		            			@SuppressWarnings("unchecked")
								Enum<?>[] enumConstants = ((Class<Enum<?>>) type).getEnumConstants();
		            			String valueAsString = (String) value;
		            			value = Arrays.asList(enumConstants).stream().filter(item -> item.name().equals(valueAsString)).findFirst().get();
		            		}
	            		}
						args.add(value);
	            	}
